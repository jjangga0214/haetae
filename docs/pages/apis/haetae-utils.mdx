...
A function to check if a file depends on one of different files, transitively or directly.<br/>
...
- `rootDir?{:ts}` : When `dependent` or an element of `dependencies` is given as a relative path, `rootDir{:ts}` is joined to transform it to an absolute path. <small>(default: [`getConfigDirname(){:ts}`](./haetae-core#getconfigdirname))</small>
...
utils.dependsOn({ dependent: 'a', dependencies, graph }) // true
utils.dependsOn({ dependent: 'c', dependencies, graph }) // true -> transitively
utils.dependsOn({ dependent: 'e', dependencies, graph }) // false
utils.dependsOn({ dependent: 'f', dependencies, graph }) // true -> 'f' depends on 'f' itself.
```
</TokenLinkCode>
