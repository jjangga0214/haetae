...
```ts
interface HaetaeRecord<D extends Rec, E extends Rec> {
  data: D
  env: E
  time: number
}
...
```ts
<D extends Rec>(recordDataFromCommand: D) => PromiseOr<D>
```
...
```ts
interface HaetaePreConfig<D extends Rec, E extends Rec> {
  commands: {
...
```ts
<D extends Rec, E extends Rec>(options: HaetaePreConfig<D, E>) => HaetaeConfig<D, E>
```
</TokenLinkCode>
...
```ts
<D extends Rec, E extends Rec>(options?: GetRecordsOptions<D, E>) => Promise<HaetaeRecord<D, E>[] | undefined>
```
</TokenLinkCode>
...
```ts
<D extends Rec, E extends Rec>(options?: GetRecordOptions<D, E>) => Promise<HaetaeRecord<D, E> | undefined>
```
</TokenLinkCode>
