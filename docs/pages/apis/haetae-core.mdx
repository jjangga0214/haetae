...
- If multiple files (of [`defaultConfigFiles`](#defaultconfigfiles)) exists,
...
<Callout>
This can override config file's `storeFile` option.<br/>
But it does not write to config file or modify [memoized](./introduction#memoization) config (by [getConfig](#getconfig)).
</Callout>
...
- `storeFile?{:ts}` : A store file path. <small>(default: [`defaultStoreFile{:ts}`](#defaultstorefile))</small>
