...
<Callout>
Memoized functions have tag chip beside their name: <Memoized/>
</Callout>

#### Cache hit and clear

To clear memoization cache, you can call `<function>.clear()`.
For example, [`getConfig`](#getconfig) is a memoized function, and `getConfig.clear()` would clear its cache.
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>(options: HaetaePreConfig<D, E>) => HaetaeConfig<D, E>`</Type>
...
- `recordRemoval.age?`: Age threshold by milliseconds. Records whose age are older than this value are to be removed when calling [`addRecord`](#addrecord).
- `recordRemoval.count?`: The number of total records to keep. When the number of records becomes larger than this value, old records are removed to satisfy the threshold when calling [`addRecord`](#addrecord). This task is executed after that of `recordRemoval.age`.
...
An argument interface of function [`getConfig`](#getconfig).
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>(options?: GetConfigOptions) => Promise<HaetaeConfig<D, E>>`</Type>
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>() => HaetaeStore<D, E>`</Type>
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>(options?: GetStoreOptions<D, E>) => Promise<HaetaeStore<D, E>>`</Type>

A function to get store object.

**Options?**

- `filename?` : A path to store file (e.g. `/path/to/.haetae/store.json`). <small>(default: [`getStoreFilename()`](#getstorefilename))</small>
- `fallback?` : A function to generate an alternative return value, when `filename` is a non-existent file. `fallback` can be useful if you want to throw an error (e.g. `({error}) => { throw error }`) rather than get a new store object from [`initNewStore`](#initnewstore) by default. <small>(default: [`initNewStore`](`#initnewstore))</small>
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>(options?: GetRecordsOptions<D, E>) => Promise<HaetaeRecord<D, E>[] | undefined> `</Type>

**Options?**

- `command?` : A command name to get records of. <small>(default: [`getCurrentCommand()`](#getcurrentcommand))</small>
- `store?` : A store object. <small>(default: [`getStore()`](`getstore()`))</small>
...
**Type**
<br/>
<Type>`<E = unknown>(options?: CommandFromConfig<unknown, E>) => Promise<E | null>`</Type>

**Options?**

- `command?` : A command name to invoke `env` of. <small>(default: [`getCurrentCommand()`](#getcurrentcommand))</small>
- `config?` : A config object. <small>(default: [`getConfig()`](#getconfig))</small>
...
**Type**
<br/>
<Type>`<D = unknown>(options?: CommandFromConfig<D, unknown>) => Promise<D | null>`</Type>
...
**Type**
<br/>
<Type>`(one: unknown, theOther: unknown): boolean`</Type>
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>(options?: GetRecordOptions<D, E>) => Promise<HaetaeRecord<D, E> | undefined>`</Type>

**Options?**

- `command?` : A command name to get records of. <small>(default: [`getCurrentCommand()`](#getcurrentcommand))</small>
- `store?` : A store object. <small>(default: [`getStore()`](#getstore))</small>
- `env?` : A resolved env object. <small>(default: [`invokeEnv({ command })`](#invokeenv))</small>
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>(options?: FormRecordOptions<D, E>) => Promise<HaetaeRecord<D, E>>`</Type>

**Options?**

- `data?` : A record data. <small>(default: [`invokeRun()`](#invokerun))</small>
- `env?` : A resolved env object. <small>(default: [`invokeEnv()`](#invokeenv)</small>
- `time?` : [Unix timestamp](https://en.wikipedia.org/wiki/Unix_time) by milliseconds. <small>(default: [`Date.now()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/now))</small>
...
**Type**
<br/>
<Type>`<D = unknown, E = unknown>(options?: AddRecordOptions<D, E>) => Promise<HaetaeStore<D, E>>`</Type>
...
- `command?` : A command name to add record under. <small>(default: [`getCurrentCommand()`](#getcurrentcommand))</small>
- `store?` : A store object. <small>(default: [`getStore()`](#getstore))</small>
- `record?` : A new record object to add. <small>(default: [`formRecord({ data: invokeRun({ command }), env: invokeEnv({ command }) })`](#formrecord))</small>
...
**Type**
<br/>
<Type>`(options?: SaveStoreOptions) => Promise<void>`</Type>

**Options?**

- `filename?` : A file path to write store on. <small>(default: [`getStoreFilename()`](#getstorefilename))</small>
- `store?` : A store object to save. <small>(default: [`addRecord()`](#addrecord))</small>
...
**Type**
<br/>
<Type>`(options?: FilenameOption) => Promise<void>`</Type>

**Options?**

- `filename?` : A store file path to delete. <small>(default: [`getStoreFilename()`](#getstorefilename))</small>
