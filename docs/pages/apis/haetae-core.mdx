...
Some functions (e.g. `getConfig`, `getStore`) are memoized, and it's explicitly specified on their description.<br/>
...
### `invokeEnv` : <Type>`<E = unknown>(options?: CommandFromConfig<unknown, E>) => Promise<E | null>`</Type>
