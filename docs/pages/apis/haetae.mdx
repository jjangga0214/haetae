...
`haetae{:ts}` also exports `cli` as API.
However, you probably don't need it.
Because, in general, the CLI features are to be used as CLI, not as API.
`haetae{:ts}` is a CLI by itself, providing exactly same features of `@haetae/cli{:ts}`.

**haetae** is the CLI bin name.
...
**ht** is its shorthand alias.

```bash
ht --help # identical to `haetae --help`
ht --version # identical to `haetae --version`
```

**For detailed usage as CLI, refer to [`@haetae/cli{:ts}`](./haetae-cli).**
...
Refer to [`@haetae/cli{:ts}`](./haetae-cli).
