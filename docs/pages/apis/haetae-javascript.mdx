...
A function to check if a module depends on one of different modules, transitively or directly.<br/>
...
- `dependent{:ts}` : A target to check if it is a dependent of at lease one of `dependencies{:ts}`, directly or transitively.
...
- `additionalGraph?{:ts}` : A graph to manually specify explicit dependency relationships. <small>(default: <TokenLinkCode tokens={{'graph' : './haetae-utils#graph'}}>`graph({ edges: [], rootDir }){:ts}`</TokenLinkCode>)</small>
...
It only can find relationship between source code files, like **.js**, **.jsx**, **.ts** or **.tsx**.<br/>
...
const additionalGraph = utils.graph({
  edges: [
    {
      dependents: ['.env.test', '.env.development'],
      dependencies: await utils.glob(['**/*.test.{ts,js}']),
    },
    {
      dependents: ['data-for-test.sql'],
      dependencies: await utils.glob([
