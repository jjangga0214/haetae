...
- **Argument**: It MAY PROBABLY work even when an argument or an option value's path delimiter is given as <code style={{'color':'var(--shiki-token-keyword)'}}>\\</code> on Windows, while not guaranteed.
Using `/{:ts}` is always officially recommended.
