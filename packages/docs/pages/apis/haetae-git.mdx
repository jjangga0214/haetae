...
- `rootDir?{:ts}` : A directory to execute shell command. <small>(default: [`core.getConfigDirname(){:ts}`](./haetae-core#getconfigdirname))</small>
...
- `from?{:ts}` : A commit ID as a starting point of comparision.
...
**When both `from` and `to` are all valid** <small>*(truthy values unlike `undefined`)*</small>

All changed files between the two commits are included.<br/>
`git diff --name-only <from> <to>{:sh}` is executed under the hood.

**When only `from` is valid**

All changed files since the commit ID `from` are included.<br/>
`git diff --name-only <from>{:sh}` is executed under the hood.<br/>
<small>(Note: This contains modified or deleted files which are [tracked](https://git-scm.com/book/en/v2/Git-Basics-Recording-Changes-to-the-Repository)
 but not commited yet, no matter whether they're [staged](https://git-scm.com/book/en/v2/Git-Basics-Recording-Changes-to-the-Repository) or not.)</small>

**When only `to` is valid**
...
<small>(Note: This does not contain files deleted in past commits because they became untracked.)</small>

**When both `from` and `to` are all invalid**
...
<Callout>
  **Usage with `dependsOn`**<br />
  [`js.dependsOn`](./haetae-javascript#dependsOn) is just an example. You can use with others, like [`utils.dependsOn`](./haetae-utils#dependsOn).
</Callout>

<TokenLinkCode tokens={{
  'configure': './haetae-core#configure',
  '.glob': './haetae-utils#glob',
  '.exec': './haetae-utils#exec',
  '.dependsOn': './haetae-javascript#dependsOn',
  '.recordData': './haetae-git#recorddata'
}}>
```js filename="haetae.config.js"
import { configure, utils, git, js } from 'haetae'
