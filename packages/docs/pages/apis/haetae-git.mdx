...
- `checkExistence?{:ts}` : Whether to filter out non-existent files from the result.
...
They don't have any intersection. A file is either *untracked* or *ignored* or *tracked*.
...
<small>(Note: This also contains modified or deleted files which're not commited yet as long as they're *tracked*, no matter whether *[staged](https://git-scm.com/book/en/v2/Git-Basics-Recording-Changes-to-the-Repository)* or not.)</small>
...
<Callout>
  **Usage with `dependsOn`**<br />
  Using with [`js.dependsOn`](./haetae-javascript#dependsOn) is just an example. You can use with others, like [`utils.dependsOn`](./haetae-utils#dependsOn).
</Callout>
