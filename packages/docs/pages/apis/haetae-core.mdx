...
<TokenLinkCode tokens={['StoreConnector', 'LocalStoreConnector', 'HaetaePreConfig', 'HaetaeConfig']}>
```ts
< D extends Rec,
  E extends Rec,
...
- `store?{:ts}` : A store connector. <small>(default: [`localStore(){:ts}`](#localstore))</small>
...
<TokenLinkCode tokens={['HaetaeConfig', 'StoreConnector']}>
```ts
...
<TokenLinkCode tokens={{
  ...
  'LoadStoreOptions': '#localstoreoptions',
  'initNewStore': '#localstoreconnectorinitnewstore',
  'loadStore': '#localstoreconnectorloadstore',
  'saveStore': '#localstoreconnectorsavestore',
}}>
```ts
...
<TokenLinkCode tokens={['LocalStoreConnector']}>
```ts
LocalStoreConnector.initNewStore
```
</TokenLinkCode>
