...
A function to calculate a hash of an env object.<br/>
...
- `env` : An `env` object to calculate a hash of.
...
<small>(default: `Number.POSITIVE_INFINITY{:ts}`) </small>
- `recordRemoval.leaveOnlyLastestPerEnv?{:ts}` : If `true{:ts}`, only the last records per `env` exist in the store file.
