...
An argument interface of the function [`setStoreFilename`](#setstorefilename).
...
A setter for the name of the store file, which is a module-level state.<br/>
...
Calling this function overrides the config file's `storeFile` option.<br/>
But it does not change the content of the config file (e.g. haetae.config.js)
or modify the [memoized](./introduction#memoization) (by [getConfig](#getconfig)) config object in memory.
</Callout>
...
- `filename{:ts}` : A store file path. If it does not end with `'.json'{:ts}`, `'store.json'{:ts}` is appended internally.
<small>(e.g. `'/path/to'{:ts}` becomes `'/path/to/store.json'{:ts}`)</small>
...
A getter for the name of the store file, which is a module-level state.<br/>
...
between each call of `configure`, which is unlikely to happen in real-world scenario.)</small>
</Callout>
...
<small>(default: `Number.POSITIVE_INFINITY{:ts}`) </small>
- `storeFile?{:ts}` : A store file path. Internally, it is used to
<TokenLinkCode inline tokens={['setStoreFilename']}>`setStoreFilename({ filename: storeFile }){:ts}`</TokenLinkCode>
...
Omitted options from a user-given config(`HaetaePreConfig`) are to be set as their default values in `HaetaeConfig`.
The given value of `storeFile` is replaced by a return value of [`getStoreFilename(){:ts}`](#getstorefilename).
...
An argument interface of the function [`getStore`](#getstore).
...
- `initWhenNotFound?{:ts}` : Whether to throw an error or just initialize new store object when `filename` does not exist in filesystem.
If `true`, [`initNewStore(){:ts}`](#initnewstore) is returned, and filesystem is not affected. <small>(default: `true{:ts}`)</small>

### `GetRecordsOptions`

An argument interface of the function [`getRecords`](#getrecords).
...
A function to get records of the given command from the given store. <br/>
...
A function to get a record of a given command from given store, corresponding to the given env.<br/>
`undefined` is returned when there is no record.
...
An argument interface of the function [`addRecord`](#addrecord).
...
- `command?{:ts}` : A command name to add record under.
<small>(default: [`getCurrentCommand(){:ts}`](#getcurrentcommand))</small>
- `store?{:ts}` : A store object. <small>(default: [`getStore(){:ts}`](#getstore))</small>
- `record?{:ts}` : A new record object to add.
<small>(default: <TokenLinkCode inline tokens={['formRecord', 'invokeRun', 'invokeEnv']}>`formRecord({ data: invokeRun({ command }), env: invokeEnv({ command }) }){:ts}`</TokenLinkCode>)</small>

### `SaveStoreOptions`

An argument interface of the function [`saveStore`](#savestore).
...
An argument interface of the function [`deleteStore`](#deleteStore).
