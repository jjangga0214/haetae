...
Refer to [the nodejs official docs](https://nodejs.org/api/child_process.html#child_processexeccommand-options-callback).
  - `trim?{:ts}` : Some commands' result (**stdout**, **stderr**) ends with whitespace(s) or line terminator character (e.g. `\n{:ts}`).
  ...
  `trim{:ts}` is the only option that is not a part of `childProcess.exec{:ts}`'s original options.
...
Basic usage is very similar to [`js.dependOn{:ts}`](./haetae-javascript#dependon),
...
<TokenLinkCode tokens={{
  '.HaetaeRecord': './haetae-core#haetaerecord',
 }}>
```ts
...
<small>([Getting Started](../getting-started) guide explains its principles.)</small>
...
- `options?{:ts}`
  ...
  If `true{:ts}`, non-existent files will just simply NOT be included in the returned result.
  If `false{:ts}`, non-existent files are given to the `hash` function, and to be included in the returned result
  if the calculated hash and previous hash are different.
  But by default, a non-existent file causes an error as hash cannot be calculated
  ...
  <small>(default: `true{:ts}`)</small>
  - `previousRecord?{:ts}` : A previous *Record Data* to compare or succeed files' hash.
  <small>(default: <TokenLinkCode tokens={{'.getRecord': './haetae-core#getrecord', 'RecordData': '#recorddata'}} inline>`core.getRecord<RecordData>(){:ts}`</TokenLinkCode>)</small>
  - `glob?{:ts}` : Whether to enable glob pattern.
  <small>(default: `true{:ts}`)</small>
