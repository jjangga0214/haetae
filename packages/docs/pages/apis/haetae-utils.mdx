...
1. [`cwd`](https://github.com/sindresorhus/execa#cwd-1) is preconfigured as [`core.getConfigDirname(){:ts}`](./haetae-core#getconfigdirname). If it's not available, `process.cwd()` is used instead.
...
- `options?{:ts}`
  ...
  <small>(default: <TokenLinkCode tokens={{'getConfig': './haetae-core#getconfig', 'RecordData': '#recorddata', 'pkg': '#pkg'}} inline>
