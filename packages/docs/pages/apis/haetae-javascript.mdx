...
A function to get version (not SemVer range, but the exact real version) of ny installed package.<br/>
...
This is purely additional, not an overrider or replacement of the original source code dependency graph. <small>(default: <TokenLinkCode tokens={{'graph' : './haetae-utils#graph'}}>`graph({ edges: [], rootDir }){:ts}`</TokenLinkCode>)</small>
