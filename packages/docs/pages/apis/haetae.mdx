...
So you don't have to explicitly return `git.initialized(){:ts}` from every command's `run` function,
when you intend to record git.

<TokenLinkCode tokens={{
  '.RootRecordData': './haetae-core#rootrecorddata',
  'RecordData': './haetae-git#recorddata',
  '.initialized': './haetae-git#initialized',
  '.recordData': './haetae-git#recorddata',
  }}>
```ts
...
The package `haetae` exports `configure`, which is different from [`core.configure`](./haetae-core#configure) exported from the package `@haetae/core`.<br/>

* For `configure`, the default value of `recordData` option is [`defaultRootRecordData`](#defaultrootrecorddata).<br/>
* For `core.configure`, the default value of `recordData` option is `(arg) => arg`. <small>(No modification of the original record data.)</small><br/>
...
<small>(The only exception is when [`core.setStoreFilename{:ts}`](./haetae-core/#setstorefilename) is called with different arguments
...
Schema of [`core.HaetaeConfig`](./haetae-core/#haetaeconfig) (return type) is ***roughly*** similar to that of [`core.HaetaePreConfig`](./haetae-core/#haetaepreconfig) (argument type).
The return value is a normalized result of user-given (argument) config.
Omitted options from user-given config(`core.HaetaePreConfig`) are to be set as their default values in `HaetaeConfig`.
Given value of `storeFile` is replaced by a return value of [`core.getStoreFilename(){:ts}`](./haetae-core/#getstorefilename).
