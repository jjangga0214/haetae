...
You can name it whatever you want. And as you might guess, you can define multiple commands (e.g. `myLint`, `myBuild`, `myAnotherTest`, etc) in the config file.<br/>
...
So there will be 3 `env`s to be (or already) recorded in the store file:
...
And this behavior is OK in general.
Configuring the removal of some history is guided later in this article.
...
import { configure, pkg, git, utils, js } from 'haetae'
...
like assertion (`assert(){:ts}`) in the example above. `myTest.env(){:ts}` is executed before `myTest.run(){:ts}`.
...
If you want to check the value the `env` function returns, you can use [`-e, --env`](./cli#-e---env) option.
...
There is a SQL file for integration test.
It's not imported (e.g. `import`, `require()`) by any source code file of course.
In this kind of case, [`utils.graph(){:ts}`](./apis/haetae-utils#graph) and
`options.additionalGraph` of [`js.dependsOn(){:ts}`](./apis/haetae-javascript#dependson) are useful.<br/>
...
The dependency graph is like this.<br/>
...
<img src="/getting-started/dependency-graph-with-sql.svg" alt="Dependency graph"/>

Let Haetae think *`logic.js`* depends on *`data.sql`*.
...
Then the dependency graph becomes explicit.<br/>
<br/>

<img src="/getting-started/dependency-graph-with-sql-focus.svg" alt="Dependency graph with .env"/>
...
2. if which test file does and doesn't depend on *`data.sql`* is not clear or changes frequently,
...
```js filename="haetae.config.js" {8}
import { configure, pkg, git, utils, js } from 'haetae'
...
export default configure({
  commands: {
    myTest: {
      env: async () => {
        return {
          sql: await utils.hash(['test/data.sql']),
        }
      },
...
<img src="/getting-started/additional-dependency-graph-cartesian-product.svg" alt="Additional Dependency Graph Cartesian Product"/>
...
<img src="/getting-started/additional-dependency-graph-distributed-notation.svg" alt="Additional Dependency Graph Distributed Notation"/>

### Circular dependency

Haetae supports circular dependency as well.
Though circular dependency is considered, in general, not a good practice,
it's fully up to you to decide whether to allow it.
Haetae does not prevent you from using it.
...
<img src="/getting-started/circular-dependency-graph.svg" alt="Circular dependency graph"/>

Assume the relations between *`index.js`*, *`foo`*, and *`bar`* are given by `additionalGraph`,
and the rest is automatically detected.

In this situation, *`index.test.js`* is executed when any of files are changed, including *`foo`*, and *`bar`*.<br/>
On the other hand, *`utils.test.js`* is executed only when *`utils.js`* or *`utils.test.js`* itself is changed.

## Record Data

Haetae has a concept of *'Record'* (type: [`HaetaeRecord`](./apis/haetae-core#haetaerecord)) and *'Record Data'* (type: `HaetaeRecord.data`).

In the previous sections, you've already seen terminal outputs like this.
...
```json filename="terminal"
{
  ...
  "result": [
    ...
    },
    {
      ...
      "env": {
        "sql": "b87b8be8df58976ee7da391635a7f45d8dc808357ff63fdcda699df937910227"
      },
      "time": 1685451151035
    }
  ]
...
For example, the command `myTest` was executed with `env: {}` on several commits, and `979f3c6` is the last commit.

TODO: how git.changedFiles works
TODO: simple custom record data

### App config ignored by git

Configuration file for your application is a good example showing the usefulness of Record Data.
...
---> `options.additionalGraph` of [`js.dependsOn(){:ts}`](./apis/haetae-javascript#dependson) or `env` are useful.
...
```js filename="haetae.config.js" {9-11,32-34}
import { configure, git, utils, js } from 'haetae'
...
export default configure({
  commands: {
    myTest: {
      ...
      run: async () => {
        const changedFiles = await git.changedFiles()
        if (previousRecord?.data?.dotenv !== (await utils.hash(['.env']))) {
...
From now on, when the file *`.env`* is changed, *`index.test.js`* and *`logic.test.js`*. are executed.
As *`util.test.js`* doesn't transitively depend on *`.env`*, it's not executed.

<Callout>
  **Hash credentials** <br/>
  [`utils.hash(){:ts}`](./apis/haetae-utils#hash) is good for credentials like a dotenv file.
  By default, it hashes by SHA-256, and you can simply change the cryptographic algorithm by its options, like to SHA-512 for example.
