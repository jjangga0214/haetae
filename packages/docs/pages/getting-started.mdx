...
<Callout>
  **More APIs not covered** <br/>
  There're more APIs, like [`js.graph{:ts}`](./apis/haetae-javascript#graph),
  [`utils.mergeGraph{:ts}`](./apis/haetae-utils#mergegraph),
  [`utils.dependsOn{:ts}`](./apis/haetae-utils#dependsOn), etc.
  This Getting Started article does not cover all of them. Check out the API docs for more.
</Callout>
...
```js filename="haetae.config.js" {9-10,31}
import { $, core, git, utils, js } from 'haetae'
...
```js filename="haetae.config.js" {9-15,22} showLineNumbers
import { $, core, git, utils, js } from 'haetae'
