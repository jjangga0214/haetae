...
Conversely, if the test fails, `pnpm jest <...>`, which we gave to [`utils.exec(){:ts}`](./apis/haetae-utils#exec)
in the config, exits with non-zero exit code.
This let [`utils.exec(){:ts}`](./apis/haetae-utils#exec) throws an error.
...
Let Haetae think *`logic.js`* depends on *`data.sql`*, by [`utils.graph(){:ts}`](./apis/haetae-utils#graph) and
`options.additionalGraph` of [`js.dependsOn(){:ts}`](./apis/haetae-javascript#dependson).
