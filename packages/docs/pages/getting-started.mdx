...
<ProjectTypeTabs>
<>
If your *`package.json`* is configured to ESM,
...
<>
If your *`package.json`* is configured to CJS,
...
If your *`package.json`* and *`tsconfig.json`* are configured to ESM,
...
If your *`package.json`* and *`tsconfig.json`* are configured to CJS,
...
<Callout>
**`recordRemoval.leaveOnlyLastestPerEnv` of [`localFileStore`](./apis/haetae-core#localfilestore)** <br/>
By default, you're using [`localFileStore`](./apis/haetae-core#localfilestore) as a *'Store Connector'*.
[`localFileStore`](./apis/haetae-core#localfilestore) stores records into a file (*`.haetae/store.json`*).
...
Check out the [API docs](./apis/haetae-utils#hash) for more detail.
