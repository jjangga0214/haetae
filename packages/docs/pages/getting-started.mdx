...
├── package.json # <--- "type": "commonjs" or "type" is omitted
├── src/ # contents are omitted for brevity
└── test/ # contents are omitted for brevity
```
</>
...
├── package.json # <--- "type": "module"
├── tsconfig.json # <--- "module": "node16" or "es6" or ... and so on
├── src/ # contents are omitted for brevity
└── test/ # contents are omitted for brevity
```

Next, install [`ts-node`](https://www.npmjs.com/package/ts-node).
It's optional `peerDependencies` of [`@heatae/core{:ts}`](./apis/core), which is an internal dependency of `haetae`.
...
<Callout>
**`stdio`**<br/>
[`stdio`](https://nodejs.org/dist/latest-v6.x/docs/api/child_process.html#child_process_options_stdio) of [`${:ts}`](./apis/haetae#dollar) is `'inherit'` by default.
This makes Promise rejected when it's [failed](https://github.com/sindresorhus/execa#childprocessresult) (e.g. non-zero exit code).
If you set `stdio` differently, you should manually make sure whether to throw an error.
...
```js filename="haetae.config.js" {5-9}
...
```js filename="haetae.config.js" {4}
// Other content is omitted for brevity
...
The default `StoreConnector` is [`core.localFileStore(){:ts}`](./apis/core#localfilestore-1),
which stores records into *`.haetae/store.json`*.
...
```ts filename="haetae.config.ts" {5-6}
import { configure, core } from 'haetae'

export default configure({
  commands: { /* ... */ },
  // Default `StoreConnector`
  store: core.localFileStore(/* You can pass custom options */),
})
```
</TokenLinkCode>

Let's implement a custom `StoreConnector` that utilizes a RDB through SQL.
...
export default configure({
  ...
  store: {
    ...
    async getRecord({ command, envHash }: core.GetRecordOptions = {}) {
      ...
      // It's allowed to be `undefined` if no previous record exists
