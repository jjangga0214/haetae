...
2. If which test file does and doesn't depend on *`data.sql`* is not clear or the relation changes frequently,
...
Though circular dependency is, in general, considered not a good practice,
...
```js filename="haetae.config.js" {9-12,33-35}
import { configure, git, utils, js } from 'haetae'
