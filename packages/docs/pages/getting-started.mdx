...
[`js.dependsOn(){:ts}`](./apis/haetae-git#dependson) detects direct or transitive dependencies between files.
...
That's because [`git.changedFiles(){:ts}`](./apis/haetae-git#changedfiles) automatically returns only the files changed since the last successful execution of Haetae.<br/>
...
## Integration test with additional dependency graph
