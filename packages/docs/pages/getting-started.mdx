...
Check out [the API docs](./apis/haetae-javascript#dependon) and pass additional option(s) if you use Typescript or Webpack.
</Callout>
...
<Callout>
  **More APIs not covered** <br/>
  There're more APIs related to dependency graph, like [`js.graph{:ts}`](./apis/haetae-javascript#graph),
  [`utils.mergeGraph{:ts}`](./apis/haetae-utils#mergegraph),
  [`utils.dependsOn{:ts}`](./apis/haetae-utils#dependson),
  [`utils.dependOn{:ts}`](./apis/haetae-utils#dependon), etc.
...
```js filename="haetae.config.js" {9-15,21} showLineNumbers
import { $, core, git, utils, js } from 'haetae'
...
By this, changes to multiple dotenv files are recorded and compared as well as `additionalGraph` works well.

[`renew`](./apis/haetae-utils#changedfiles) is a list of files that will be renewed(if changed) by their current hash.
By default, `renew` is equal to all files in the argument.
In our config, by limiting it to `.env.${process.env.ENV}{:ts}`, you only renew the single dotenv file.
For example, let's say currently `$ENV` is `'local'`.
Obviously, *`.env.local`*, *`.env.development`*, and *`.env.staging`* are compared to the previous hashes to detect if changed.
But *`.env.development`*, and *`.env.staging`* are not renewed in the new Record Data.
Their previous hashes are recorded instead of current hashes, no matter whether the current ones are different from the previous ones.
Only *`.env.local`* will be renewed.
This is good for our test.
For instance, *`.env.development`* can be changed when `$ENV` is `'local'`.
And the detection of the change to *`.env.development`* is kept until `$ENV` changes to `'development'`.
So when `$ENV` becomes `'development'`, [`utils.changedFiles`](./apis/haetae-utils#changedfiles)
would think *`.env.development`* is still a changed file,
though the change is, in fact, made in the past.
This lets test files that transitively depend on the dotenv file executed when `$ENV` becomes `'development'`.
...
[`filterByExistence`](./apis/haetae-utils#changedfiles) is `false{:ts}` by default for intended safety.
If so, all given files will be passed to `hash` function internally.
If a non-existent file is given, the `hash` function won't be able to read the file.
So it throws an error, unless you give a non-default hash function that can handle this situation as an option.

If [`filterByExistence`](./apis/haetae-utils#changedfiles) is `true{:ts}`,
a non-existent file will be treated as not changed.
So it's just simply not included in the returned result without an error.
And the hash of the non-existent file succeeds the previous record,
which means recorded as the same value as the previous one from the previous *Record Data*.
If the previous *Record Data* doesn't contain the file, the hash will not be recorded.

When not all of the files might exist on the filesystem at the same time
and only a few of them are dynamically used in incremental tasks,
`filterByExistence: true{:ts}` and `renew` are useful.
...
`filterByExistence: true{:ts}` and `renew` are good for such a scenario.
<small>(Haetae on CI is guided in 'CI' article.)</small>
...
So, if `$ENV` is changed, the previous one will no longer be recorded.
Therefore you should list all of the candidates.<br/>
Like `['.env.local', '.env.development', '.env.staging']{:ts}`.
