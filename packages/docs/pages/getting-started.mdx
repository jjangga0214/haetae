...
<Callout>
**`filterByExistence`** <br/>
Note that the option `filterByExistence: true{:ts}` on line number 11 is needed
only if not all of *`.env.local`*, *`.env.development`*, and *`.env.staging`* may exist simultaneously in the filesystem.
It's `false{:ts}` by default for safety. By default, a non-existent file causes an error as hash cannot be calculated
unless you give a non-default hash function that can handle this situation as another option.
If `filterByExistence: true{:ts}`, non-existent files will just simply not be included in the returned result without an error.
</Callout>

But you should NOT give a dynamic argument to [`utils.changedFiles{:ts}`](./apis/haetae-utils#changedfiles).
Otherwise, a file would be treated as changed every time dynamic argument changes.
...
For [`utils.changedFiles{:ts}`](./apis/haetae-utils#changedfiles) to work, the list of files should be recorded consistently.
Therefore you should list all of the candidates, like `['.env.local', '.env.development', '.env.staging']{:ts}`.
