...
```js filename="haetae.config.js" {10-13,34-36}
import { configure, git, utils, js } from 'haetae'
...
The value we returned from `myTest.run{:ts}` is recorded in the store file.
...
This time, *`index.test.js`* and *`logic.test.js`* are not executed, as *`.env`* is treated as not changed.
